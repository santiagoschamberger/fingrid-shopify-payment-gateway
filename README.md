# Fingrid Payment Gateway - Shopify App

This is a Shopify app that integrates the Fingrid payment gateway, allowing merchants to accept bank transfer payments with optional discounts. Built using the [Remix](https://remix.run) framework and Shopify's App Bridge.

## Features

- 🏦 **Bank Transfer Payments**: Direct bank account payments through Fingrid's secure API
- 💾 **Metafields Storage**: Uses Shopify's native metafields for data storage (no external database required)
- 🔐 **Secure Token Management**: Encrypted storage of sensitive payment credentials
- 📱 **Checkout UI Extension**: Seamless payment method integration in Shopify checkout
- 👤 **Customer Account Management**: Bank account management in customer accounts
- 📊 **Admin Dashboard**: Transaction reporting and settings management
- 🎯 **Discount Support**: Configurable discounts for bank transfer payments
...
4. **Fingrid API Credentials**: Contact Fingrid to obtain test and production API credentials
...
2. **Shopify App Configuration**: The app is already configured with the required scopes and webhooks in `shopify.app.toml`.

3. **Fingrid Settings**: After installation, configure your Fingrid credentials in the app settings:
   - Navigate to your Shopify admin
   - Go to Apps > Fingrid Payment Gateway > Settings
   - Configure test/production credentials and settings
